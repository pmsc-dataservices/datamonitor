$(document).ready(function(){
	...
	$('ul.main-menu li a').each(function(){
		if($($(this))[0].href==String(window.location)){
			...
			if($(this).parents(".second-menu")){
				$(this).parents(".second-menu").show();
			}
		}
	});
	...
	$('ul.main-menu li:not(.nav-header)').hover(function(){
		$(this).children("a").animate({'padding-left':'+=5'},300);
	},
	function(){
		$(this).children("a").animate({'padding-left':'-=5'},300);
	});
			...
			for ( i=0, iLen=an.length ; i<iLen ; i++ ) {
				...
				for ( j=iStart ; j<=iEnd ; j++ ) {
					...
					$('<li '+sClass+'><a href="#">'+j+'</a></li>')
						.insertAfter( $('li:eq('+index+')', an[i])[0] )
				...
				var next_to_last = $('li', an[i]).length-3;
				if ( oPaging.iPage === oPaging.iTotalPages-1 || oPaging.iTotalPages === 0 ) {
					
					$('li:gt('+next_to_last+')', an[i]).addClass('disabled');
				} else {
					$('li:gt('+next_to_last+')', an[i]).removeClass('disabled');
				}
			}
		}
	}
